...
from app.models import Student, User, Assessment, CategoryEnum, db
...
def create_students():
    """
    Accepts either a single student JSON object or an array of students for bulk creation.
    """
    user_id = get_jwt_identity()
    user = User.query.get(user_id)
    ...
    for item in students_data:
        ...
        try:
            ...
            return jsonify({"error": f"Invalid category value: {item['category']}"}), 400

        pe_value = item.get('physical_education', False)
        if isinstance(pe_value, str):
            pe_value = pe_value.lower() in ['true', '1', 'yes']
...
def update_student(student_id):
    ...
    if 'physical_education' in data:
        ...
        if isinstance(pe_val, str):
            pe_val = pe_val.lower() in ['true', '1', 'yes']
